...
RUN /bin/sh -c wget ftp://ftp.ncbi.nlm.nih.gov/blast/executables/blast+/2.11.0/ncbi-blast-2.11.0+-x64-linux.tar.gz -O /tmp/blast.tar.gz && tar zxvf /tmp/blast.tar.gz -C /opt/ && rm /tmp/blast.tar.gz

ENV PATH /opt/conda/envs/dolphinnext-barcodeseq-1.0/bin:$PATH:/opt/ncbi-blast-2.10.0+/bin
